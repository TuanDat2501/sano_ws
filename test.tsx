...
export default function KanbanBoard() {
  ...
  const handleClearDoneTasks = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (socket) socket.emit("board_updated"); // Báo các máy khác load theo
      } else {
  ...
  const loadProjects = async () => {
    try {
      const res = await fetch("/api/projects"); // Thay bằng API thật của sếp
      const data = await res.json();
  ...
  // Hàm xử lý khi QLK ấn chốt điểm
  const handleEvaluationSubmit = async (score: number, criteriaData: any, note: string) => {
    ...
    try {
      const res = await fetch(`/api/tasks/${selectedTask.id}/evaluate`, {
        ...
        body: JSON.stringify({ score, criteriaData, note }) 
      });
      ...
      if (res.ok) {
        ...
        showToast('success', isPass ? `Đã duyệt! Video đạt ${score} điểm.` : `Đã trả về (Điểm: ${score}). Yêu cầu Kaizen!`);
        
        // Cập nhật lại board Kanban
        fetchTasks();
        setIsDrawerOpen(false); 
        handleCloseDrawer();

        // 🚀 TỰ ĐỘNG HÓA: Bắn Chat & Notify qua Socket
        if (socket) {
           // Bắn tin nhắn tự động của BOT vào group chat
           if (data.systemComment) {
              const newMsg = {
                id: data.systemComment.id, taskId: selectedTask.id, sender: "Hệ thống QA",
                senderId: "SYSTEM", text: data.systemComment.text,
                time: new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }),
              };
              socket.emit("send_message", newMsg);
           }
           // Trigger load lại toàn bộ bảng cho các nhân sự khác
           socket.emit("board_updated");
        }
      } else {
        showToast('error', 'Có lỗi khi lưu kết quả chấm điểm!');
      }
    } catch (error) {
       showToast('error', 'Lỗi kết nối Server!');
    }
