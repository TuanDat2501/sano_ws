...
export default function DailyReportPage() {
    ...
    return (
        ...
        {/* 🚀 1. Khóa view 1 màn hình với max-h và overflow-hidden */}
        <div className="h-full max-h-[calc(100vh-80px)] flex flex-col p-4 md:p-6 bg-slate-50 overflow-hidden animate-fade-in">
            
            {/* 🚀 2. Header giữ nguyên kích thước (shrink-0) và gộp vào 1 khối trắng cho đồng bộ UI */}
            <div className="mb-4 md:mb-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 shrink-0 bg-white p-4 rounded-2xl shadow-sm border border-slate-200 relative z-10">
                <div>
                ...
                {/* THỐNG KÊ NHANH */}
                <div className="flex justify-between w-full xl:w-auto bg-slate-50 p-2.5 md:p-3 rounded-xl border border-slate-200 shadow-inner">
                    <div className="text-center px-2 md:px-4 border-r border-slate-200 flex-1 xl:flex-none">
                        <p className="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tổng nhân sự</p>
                        <p className="text-lg md:text-xl font-black text-slate-800">{totalUsers}</p>
                    </div>
                    <div className="text-center px-2 md:px-4 border-r border-slate-200 flex-1 xl:flex-none">
                        <p className="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">Đã báo cáo</p>
                    ...
                    <div className="text-center px-2 md:px-4 flex-1 xl:flex-none">
                        <p className="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">Chưa báo cáo</p>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex-1 overflow-auto custom-scrollbar relative z-0">
                <table className="w-full text-left border-collapse min-w-[550px] md:min-w-full">
                    
                    {/* 🚀 4. Ghim Header lên Top (sticky top-0 z-20) */}
                    <thead className="sticky top-0 z-20 bg-slate-100/95 backdrop-blur-md shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                        <tr>
                            <th className="p-4 font-black text-slate-500 uppercase tracking-widest text-[10px] md:text-xs w-1/3 border-b border-slate-200">Nhân sự</th>
                            <th className="p-4 font-black text-slate-500 uppercase tracking-widest text-[10px] md:text-xs w-1/4 border-b border-slate-200">Trạng thái</th>
                            <th className="p-4 font-black text-slate-500 uppercase tracking-widest text-[10px] md:text-xs border-b border-slate-200">Link đính kèm trong ngày</th>
                        </tr>
                    </thead>
                            ...
                            >
                                ...
                                <td className="p-3 md:p-4">
                                    <div className="flex items-center gap-2.5 md:gap-3">
                                        <div className="h-8 w-8 md:h-10 md:w-10 bg-slate-200 rounded-full flex items-center justify-center font-black text-slate-600 overflow-hidden shrink-0 text-xs md:text-base border-2 border-white shadow-sm">
                                            {user.avatarUrl ? (
                                ...
                                <td className="p-3 md:p-4">
                                    {user.hasReported ? (
                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1 rounded-full bg-green-100 text-green-700 text-[10px] md:text-xs font-bold border border-green-200">
                                            <CheckCircle2 size={14} className="md:w-4 md:h-4" /> Đã cập nhật
                                        </span>
                                    ) : (
                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1 rounded-full bg-red-100 text-red-700 text-[10px] md:text-xs font-bold border border-red-200 animate-pulse shadow-sm">
                                            <AlertCircle size={14} className="md:w-4 md:h-4" /> Trống báo cáo
                                        </span>
                                    )}
                                </td>
